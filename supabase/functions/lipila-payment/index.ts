...
serve(async (req) => {
  ...
  if (payErr || !payment) {
    console.error('[lipila] DB insert failed — code:', payErr?.code, 'msg:', payErr?.message, 'details:', payErr?.details, 'hint:', payErr?.hint, 'method:', payment_method, 'type:', payment_type, 'ikey:', idempotency_key);
    return json({ error: 'Payment initiation failed. Please try again.' }, 500);
  }
